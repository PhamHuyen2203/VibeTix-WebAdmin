...
export const createPromotion = onCall({ region: 'asia-southeast1' }, async (request) => {
    ...
    // Audit log
    await writeAuditLog(admin.uid, admin.displayName, 'promotion.create', 'promotion', docRef.id, { code: codeNormalized, type: data.type, value: data.value });
